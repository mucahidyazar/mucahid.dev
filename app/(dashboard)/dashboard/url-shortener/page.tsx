...
export default function LinkPreviewPage() {
  ...
  const {
    data: urlShortenerData,
    mutate: myUrlShortenerMutate,
    ...
    <li className='border-b border-solid border-indigo-200 border-opacity-20 bg-indigo-500 bg-opacity-20 rounded'>
      <div className='flex flex-col gap-1 p-2'>
        <div className='flex items-center gap-2'>
          <Tooltip content={hasCopied ? 'Copied' : 'Copy'}>
            <DocumentDuplicateIcon
              ...
              onClick={() => {
                navigator.clipboard.writeText(`http://localhost:3000/s/${data.short}`)
                setHasCopied(true)
        ...
        <div className='text-xs text-gray-400'>
          <span className='font-semibold inline-block w-10'>Clicks</span>{data.clicks}
        </div>
        <div className='text-xs text-gray-400'>
          <span className='font-semibold inline-block w-10'>Link</span>{data.full}
        </div>
        <div className='text-xs text-gray-400'>
